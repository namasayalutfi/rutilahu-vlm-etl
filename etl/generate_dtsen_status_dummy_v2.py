import json
import random
from dataclasses import dataclass
from pathlib import Path
from typing import Dict, List, Any, Set, Optional


@dataclass
class DTSENDummyConfig:
    input_path: Path = Path("metadata/reconciled_metadata_mkn2.json")
    changed_ids_path: Path = Path("metadata/changed_house_ids.txt")
    output_path: Path = Path("metadata/mkn2_metadata_new.json")
    changed_output_path: Path = Path("metadata/mkn2_metadata_changed_only.json")
    seed: int = 42
    same_probability: float = 0.7


class DTSENDummyGenerator:
    LABELS = {
        "atap": [
            "Beton",
            "Genteng",
            "Seng",
            "Asbes",
            "Bambu",
            "Kayu/sirap",
            "Jerami/ijuk/daun-daunan/rumbia",
            "Lainnya",
        ],
        "dinding": [
            "Tembok",
            "Plesteran anyaman bambu/kawat",
            "Kayu/papan/gypsum/GRC/calciboard",
            "Anyaman bambu",
            "Batang kayu",
            "Bambu",
            "Lainnya",
        ],
        "lantai": [
            "Marmer/granit",
            "Keramik",
            "Parket/vinil/karpet",
            "Ubin/tegel/teraso",
            "Kayu/papan",
            "Semen/bata merah",
            "Bambu",
            "Tanah",
            "Lainnya",
        ],
    }

    def __init__(self, config: DTSENDummyConfig):
        self.config = config
        self.rng = random.Random(config.seed)

    @staticmethod
    def load_json(path: Path) -> List[Dict[str, Any]]:
        if not path.exists():
            raise FileNotFoundError(f"File tidak ditemukan: {path}")
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        if not isinstance(data, list):
            raise ValueError("Input JSON harus berupa list of records.")
        return data

    @staticmethod
    def load_changed_ids(path: Path) -> Set[str]:
        if not path.exists():
            raise FileNotFoundError(f"File tidak ditemukan: {path}")

        changed_ids: Set[str] = set()
        with open(path, "r", encoding="utf-8") as f:
            for line in f:
                hid = line.strip()
                if hid:
                    changed_ids.add(hid)
        return changed_ids

    @staticmethod
    def save_json(data: List[Dict[str, Any]], path: Path) -> None:
        path.parent.mkdir(parents=True, exist_ok=True)
        with open(path, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=2, ensure_ascii=False)

    def generate_dtsen_label(self, actual: Optional[str], component: str) -> str:
        candidates = self.LABELS[component]

        # actual boleh "Tidak terdeteksi", tetapi dtsen tidak boleh punya label itu
        if actual == "Tidak terdeteksi" or actual is None:
            return self.rng.choice(candidates)

        if self.rng.random() < self.config.same_probability:
            return actual

        other = [x for x in candidates if x != actual]
        return self.rng.choice(other)

    @staticmethod
    def generate_status(actual: Optional[str], dtsen: str) -> str:
        if actual == "Tidak terdeteksi" or actual is None:
            return "Tidak teridentifikasi"
        if actual == dtsen:
            return "Sesuai"
        return "Tidak sesuai"

    def process_record(self, record: Dict[str, Any]) -> Dict[str, Any]:
        actual = record.get("actual_label", {})
        if not isinstance(actual, dict):
            actual = {}

        dtsen: Dict[str, str] = {}
        status: Dict[str, str] = {}

        for component in ["atap", "dinding", "lantai"]:
            actual_value = actual.get(component, "Tidak terdeteksi")
            if actual_value is None:
                actual_value = "Tidak terdeteksi"

            dtsen_value = self.generate_dtsen_label(actual_value, component)
            dtsen[component] = dtsen_value
            status[component] = self.generate_status(actual_value, dtsen_value)

        updated = dict(record)
        updated["dtsen"] = dtsen
        updated["status"] = status
        return updated

    def run(self) -> Dict[str, Any]:
        data = self.load_json(self.config.input_path)
        changed_ids = self.load_changed_ids(self.config.changed_ids_path)

        full_output: List[Dict[str, Any]] = []
        changed_only_output: List[Dict[str, Any]] = []

        summary = {
            "Sesuai": 0,
            "Tidak sesuai": 0,
            "Tidak teridentifikasi": 0,
        }

        processed = 0
        skipped = 0

        for record in data:
            house_id = str(record.get("house_id", "")).strip()

            if house_id in changed_ids:
                updated = self.process_record(record)
                processed += 1
                changed_only_output.append(updated)
            else:
                updated = dict(record)
                skipped += 1

            full_output.append(updated)

            status_obj = updated.get("status", {})
            if isinstance(status_obj, dict):
                for value in status_obj.values():
                    if value in summary:
                        summary[value] += 1

        self.save_json(full_output, self.config.output_path)
        self.save_json(changed_only_output, self.config.changed_output_path)

        return {
            "total_house": len(full_output),
            "processed_house": processed,
            "skipped_house": skipped,
            "changed_only_house": len(changed_only_output),
            "output_path": str(self.config.output_path),
            "changed_output_path": str(self.config.changed_output_path),
            **summary,
        }


if __name__ == "__main__":
    generator = DTSENDummyGenerator(DTSENDummyConfig())
    result = generator.run()

    print("[OK] Generate DTSEN dummy selesai.")
    for k, v in result.items():
        print(f"{k}: {v}")