...
class ReconcileConfig:
    metadata_path: Path = Path("metadata/reconciled_metadata.json")
    labelstudio_output_path: Path = Path("data/data_test_ls_output.json")
    output_json_path: Path = Path("metadata/reconciled_metadata_mkn2.json")
...
class LabelStudioMetadataReconciler:
    ...
    def _resolve_anomaly_mode(anomaly_flags: Set[str]) -> str:
        """
        anomali_1: multi, foto exterior & interior tertukar posisi slotnya
                   -> view_type asli setiap slot adalah KEBALIKAN dari slotnya
                      (slot ext_img sebenarnya interior, slot int_img sebenarnya exterior)
        anomali_2: single, foto yang dipertahankan view_type aslinya kebalikan dari slotnya
                   (single_interior_only tapi foto int_img sebenarnya exterior, atau
                    single_exterior_only tapi foto ext_img sebenarnya interior)
        Kedua kasus di atas secara teknis sama-sama berarti "balik view_type default slot".
        """
    ...
    def _collect_kept_images(
        ...
        # anomali_1 (swap_slots) dan anomali_2 (flip_kept_images) sama-sama berarti:
        # view_type final harus dibalik dari mapping default slot (ext->exterior, int->interior)
        needs_flip = anomaly_mode in ("swap_slots", "flip_kept_images")
        ...
        for slot in slots:
            ...
            default_view = "exterior" if prefix == "ext" else "interior"
            cloned["view_type"] = self._flip_view_type(default_view) if needs_flip else default_view
    ...
    def reconcile(self) -> Dict[str, Any]:
        ...
        # Iterasi langsung di atas seluruh record metadata sumber.
        # House id yang TIDAK ada di labelstudio_output dibiarkan apa adanya (tidak dihapus/skip).
        for source_record in sample_records:
            house_id = str(source_record.get("house_id", "")).strip()
            ls_item = labelstudio_index.get(house_id) if house_id else None

            if ls_item is None:
                reconciled_records.append(source_record)
                continue
            ...
            reconciled_records.append(
                {
                    ...
                    "dtsen": source_record.get("dtsen", self._null_status_object()),
                    "status": source_record.get("status", self._null_status_object()),
                }
            )
